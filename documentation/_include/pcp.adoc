...
Let's start by clicking "Select" next to PCP Redis. This will bring us to the following configuration page:

image::grafana-45-pcpredis-config.png[PCP Redis Config]

For the hostname, we will specify `http://localhost:44322`:

image::grafana-50-saveandtest.png[Eh]

Now we'll hit "Save & Test":

image::grafana-55-addagain.png[Eh]
