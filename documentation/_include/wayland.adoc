...
NOTE: The graphical desktop is disabled by default to improve the overall performance of this workshop in the public cloud.  If you have access to the console, you can switch on the graphical desktop with the following command run as the root user: `systemctl isolate graphical.target`
