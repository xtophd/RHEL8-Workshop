...
For this exercise, you will be using the host `node1`

From host `workstation`, you should be able to ssh to `node1` without any prompts for credentials.
...
Now that we have a good idea of what’s installed on our system, let’s get a basic lamp stack up and running.  If you prefer not to type all of these commands, there's a cheat script below to save you some time.
