...
Each one of those entries makes up a chunk of a session recording.  It is by way of searching and itentifying the correct TLOG_REC for a session, that you can then play back the session using `tlog-play`.

.[root@node1]#
----
tlog-play -r journal -M TLOG_REC=b2333248c22a4c5f912a245f6266582e-3cb9-e9bf9
----

Once you hit enter on the above command, notice that your terminal is now playing back the session log in all of it's glory.  Be patient until your root prompt returns.
