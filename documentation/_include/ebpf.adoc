...
Let's open another workstation terminal session. To do that, from tmux, hit `Ctrl+b` then the `"` key. This should split the window into two panes and give you a new root session on node2. Now you need to run:
...
Now from this workstation terminal session, run:
...
To switch back to the pane running `tcplife`, hit `Ctrl+b` and then press `o`. You can use this shortcut to switch between panes at any time with tmux.
...
NOTE: You are not required to reference any additional resources for these exercises.  This is informational only.
    ...
    * link:https://github.com/xdp-project/xdp-tutorial[Upstream XDP Tutorial (networking)]
