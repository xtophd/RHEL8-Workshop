...
For these exercises, you will primarily be using the host `node2` as user `root`.  However, some exercises will need a second teminal session connected to `workstation`.

TIP: Now is a great time to use the multi session capability of *tmux*.  Use `Ctrl-b c` to create another session.  You can now cycle back and forth between them with `CTRL-b n` (next) and `CTRL-b p` (previous).
...
In your second terminal, establish yourself as described above and then run the following command:
...
Again, in you second terminal as user `root` on `workstation`, run the following:
...
*PAY ATTENTION!!!* This time in you second terminal as user `root` on host `node2`.
...
You now see that this umount succeeded. 
...
Go to the terminal with `mountsnoop` running and issue a Ctrl-C. This will return you to your prompt.
