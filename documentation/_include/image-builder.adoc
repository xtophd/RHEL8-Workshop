...
Your image is now mostly ready for deployment.  To perform as an actual webserver, we still need to add actual content to the image and also set the httpd service to 'enabled'.  We will tackle those steps in the next unit covering virtualization on RHEL with kvm.
