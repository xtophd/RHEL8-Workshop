...
Now of course, leveraging additional services like pxe, dhcp, kickstart, ansible automation, etc... The process can be heavily automated.
...
`Image Builder` is a set of tools that create custom RHEL images in a variety of formats for compatibility with major cloud providers and virtualization technologies.  Meaning, you can create a blueprint of the image and then specify the target platfrom to create an appropriate image for VMWare, AWS, Openstack, KVM, etc...
