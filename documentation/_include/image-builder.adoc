...
Looking over the bludprint, it should be apparent that image builds on a standard RHEL base by:
...
Your image is now mostly ready for deployment.  We say "mostly" because there are still a few steps to do before our image performs as an actual webserver:

    * set the httpd service to 'enabled'.
    * add actual content to the image 

We will tackle those steps in the next unit covering virtualization on RHEL with kvm.
