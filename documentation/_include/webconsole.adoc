...
The Web Console listens on port 9090 of the host.  To make the system more accessible to students, an additional port of 443 was added to the configuration.  This was done by creating a systemd drop-in config:
...
This also required a systemctl daemon-reload, selinux update and firewall rule addition.  Remember this provided as reference only.
