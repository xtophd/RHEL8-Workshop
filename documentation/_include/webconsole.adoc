...
For your convenience, the webconsole has already been installed and configured on *bastion.example.com*.  The steps for installation and setup are pretty simple and outlined below.  Remember that this is provided as reference only.
...
Accept the various notifications about insecure ceritificates, and then login in.
  ...
  * URL: link:https://{targethost_fqdn}/[] 
  * UserID: %ssh_username%
...
In the dialog, filter on 'ntp' and then select the 'ntp' check box.  Lastly hit "Add services".
...
As of RHEL 8.4, the "Web Console Dashboard" plug-in has been deplricated.  The core functionality of being able to manage multiple nodes from a single interface has been intergrated into the webconsole base.  Now adding additional nodes and selecting one to manage is simple and intuitive.
...
Add system 'node1.%subdomain'.

Repeat the steps to add node2 and node3.
