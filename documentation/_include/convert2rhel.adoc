...
[source,options="nowrap",subs="{markup-in-source}",role="copy"]
----
curl --insecure --output /usr/share/convert2rhel/subscription-manager/katello-ca-consumer-latest.noarch.rpm http://satellite.example.com/pub/katello-ca-consumer-latest.noarch.rpm
----

[source,options="nowrap",subs="{markup-in-source}",role="copy"]
----
convert2rhel --org ExampleCom --activationkey MyKey-DEV
----
