...
You may a one or more 2 zones depending on the deployed host and it's configuration:

  * `public` zone on interface `eth0`
  * `libvirt` zone on interface `virbr0`
  
NOTE:  In this sample output, the virtual bridge `libvirt` is created and managed by libvirtd.  It is possible that your system will not the libvirt zone.  For our purposes, we are only interested in the public zone and the interface `eth0`.
