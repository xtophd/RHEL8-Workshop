...
Take a look at the active services now and you should find dhp6-client absent.
...
Again, we point out that what we just did is not permanent (ie: these changes will not persist after a reboot).  Have a look at the output from `firewall-cmd --list-services --permanent*`.  See the difference?  Our unwanted service will return after a `firewall-command --reload` or a system reboot.
...
Thus, there is one more step.  Save our current active configuration.
