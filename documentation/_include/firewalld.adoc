...
Again, we point out that what we just did is not permanent (ie: these changes will not persist after a reboot).  Have a look at th eoutput from *--list-services --permanent*.  See the difference?  Our unwanted service will return after a *firewall-command --reload* or a system reboot.
